...
** 20070301
*** Android项目
1	S700带起了方糖CB6版型， 发给瑞泰做车机方案第一阶段
2	使用GS900增加96 board版型（TAG 0227）， 使用该分支和版型做后续S900推广。
3  瑞泰第一阶段需求评估完成， 第二阶段主要工作在双系统和U盘量产。
4  拜访了视美泰，了解多很多商显的需求， 初步看工作量比较大。
5  Google DayDream中的APK从手机上抠出来，经过debug可以在5209上跑起来，IMG GPU的部分VR扩展还不能用，沟通中，进展较慢。
6  Android 7.0 GMS，目前了解到的情况需要A53等Arm V8架构的cpu才可以支持TEE。 A5 A7 A9的CPU应该不能过。
7  差异化和VR的支持工作。（Linux X11选择， HDMI IN，KODI小窗口等需求）	支持S700
支持工作
...
*** 5119
IVT第一阶段代码完成， IVT库从130K压缩到110，第二阶段的工作， 需要从应用，IVT库以及控制器各方代码一起联动， 后续由柯海涛接手。	1.	Rom固化代码预计在3/10前整合完成。之后安排代码review。
2.	Eeprom方案SDK预计在在3/10提供，外部开发人员可进入。
3.	Spinor方案开发SDK预计在3/17提供。
...
12.	串口外设支持（包括USB转TTL，PL2303，FT2303等）：
POS打印机，
读卡器等外设。
...
30.4	声音大小，喇叭，耳机，3G
30.5	声音输出通道默认配置
30.6	USB HID的VID，PID设置
30.7	USB触摸屏原点位置
30.8	IIC电容屏配置，IC芯片，地址，大小分辨率，原点位置（为了兼容多家IIC触摸不造成软件分支）
30.9	WIFI AP默认名称和密码配置
30.10    遥控器机器码厂家配置
30.11    3G模块名称配置。（为了兼容多家3G模块不造成软件分支）
30.12    是否支持3G模块选择。（为了支持多串口，兼容USB转TTL，PL2303，FT2303等）
30.13    调试背光频率，相位和默认背光强度。
31.	一键恢复出厂设置。
