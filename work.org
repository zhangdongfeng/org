...
+ btmgr_start(btmgr_ready_cb);
  ...
  + err = ivtmgr_drv_init(ivtmgr_drv_rx_cb);
    ...
    + k_thread_spawn(g_schedule_loop_stack,
                      sizeof(g_schedule_loop_stack),
                      schedule_loop_thread, NULL, NULL,
    + k_thread_spawn(g_schedule_loop_low_stack,
                       sizeof(g_schedule_loop_low_stack),
                       schedule_loop_low_thread, NULL, NULL,
  + g_ready_cb = cb;
  + err = btstack_init();
...
+ btmgr_ready_cb
  + btmgr_a2dp_start(&a2dp_cbs);
    ...
    + A2DP_RegCbk(in);
    + p_a2dp->snk_svc_hdl = A2DP_RegisterService(A2DP_TSEP_SNK, A2DP_SPEAKER);
