...
** aihora
*** AB1520S
**** AB1522S headset configuration tool
AB1522S Headset configuration tool provides to customers the capability to configure
the various settings, such as Button I/O, LED behavior, Key operations, and Battery Parameters stored in flash memory. Customer code also can be processed by Airoha’s AB1522S Headset configuration tool. Customers only need to prepare and compile their program by C compiler, and use the tool to automatically generate the binary data into flash memory.
**** AB1522S Mass-Production tool
 AB1522S Mass-Production tool is used during customer production stage and it
provides the capability for downloading/verifying flash memory, testing RF performance, ADC calibration, and updating Bluetooth device address of the headset module. It works with the Airoha provided Test Control Board (TCB) hardware. The Mass-Production tool greatly improves the test and verification efficiency in manufacturing stage.

AB1522S Stereo Solution for Intensive
Audio Applications Page 34 of 36 CONFIDENTIAL Preliminary Specification
Version 0.11 Jul-2016
Confidential
